import React from 'react';
import PropTypes from 'prop-types';

const CloseIcon = ({ classes }) => (
  <svg
    viewBox="0 0 30 30"
    fill="currentColor"
    xmlns="http://www.w3.org/2000/svg"
    className={classes}
  >
    <path d="M.82 4.96l24.218 24.22c1.114 1.114 2.947 1.09 4.089-.053 1.143-1.143 1.167-2.974.052-4.088L4.962.819C3.848-.292 2.015-.27.873.873-.27 2.016-.294 3.847.821 4.961z" />
    <path d="M4.961 29.18L29.18 4.961c1.114-1.114 1.09-2.947-.052-4.089-1.144-1.143-2.975-1.167-4.09-.052L.82 25.038c-1.114 1.114-1.09 2.947.053 4.089 1.143 1.143 2.974 1.167 4.088.052z" />
  </svg>
);

CloseIcon.propTypes = {
  classes: PropTypes.string.isRequired
};

export default CloseIcon;
